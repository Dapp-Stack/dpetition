...
contract DaiFab is DSAuth {
    ...
    function makeVoxTub(IERC20 gem, DSToken gov, DSValue pip, DSValue pep, address pit) public auth {
        ...
        require(address(pit) != address(0x0));
        vox = voxFab.newVox();
    ...
    function configAuth(DSAuthority authority) public auth {
        ...
        dad.permit(address(top), address(tub), S("cage(uint256)"));

        dad.permit(address(top), address(tub), S('mint(address,uint256)'));
        dad.permit(address(top), address(tub), S('burn(address,uint256)'));
        ...
        dad.permit(address(top), address(tub), S('mint(address,uint256)'));
        dad.permit(address(top), address(tub), S('burn(address,uint256)'));
        dad.permit(address(top), address(tub), S('burn(uint256)'));
        dad.permit(address(top), address(tub), S('burn(uint256)'));

        dad.permit(address(top), address(tub), S('mint(uint256)'));
        dad.permit(address(top), address(tub), S('burn(uint256)'));
        dad.permit(address(top), address(tub), S('burn(address,uint256)'));

        dad.permit(address(top), address(tub), S("mold(bytes32,uint256)"));
        dad.permit(address(top), address(tub), S("tune(uint256)"));
        dad.permit(address(top), address(tub), S("mold(bytes32,uint256)"));
        dad.permit(address(top), address(tub), S("mold(bytes32,uint256)"));
        dad.permit(address(top), address(tub), S("setPip(address)"));
        dad.permit(address(top), address(tub), S("setPep(address)"));
        dad.permit(address(top), address(tub), S("setVox(address)"));
